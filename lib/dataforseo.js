...
async function dataForSeoFetch(path, options = {}) {
  ...
  const timeout = setTimeout(() => controller.abort(), intEnv('DATAFORSEO_FETCH_TIMEOUT_MS', DEFAULT_FETCH_TIMEOUT_MS));
  let response;

  try {
    response = await fetch(`${DATAFORSEO_BASE_URL}${path}`, {
      ...options,
    ...
    if (error.name === 'AbortError') {
      const timeoutError = new Error('DataForSEO request timed out');
